...
_K='fcn_resnet101'
_J='fcn'
_I='deeplabv3_resnet50'
_H='deeplabv3_resnet101'
_G='deeplabv3_mobilenet_v3_large'
_F='deeplabv3'
_E='lraspp_mobilenet_v3_large'
_D='Accuracy (%)'
_C=False
_B='Execution Time (s)'
_A='Model'
...
available_models=[_F,_G,_H,_I,_J,_K,_L,_M,_E]
model_name=_E
...
def getTransform(image_folder):
	...
	for image_name in os.listdir(image_folder):
		if image_name.lower().endswith(('.jpg','.jpeg','.png')):image_path=os.path.join(image_folder,image_name);image=Image.open(image_path).convert(_N);dimensions.append(image.size);image=np.array(image);pixel_values.append(image)
...
model.eval()
def save_image(extracted_image,image_name,test=_C):
...
def show_image(original_image,extracted_image,accuracy_percentage=''):A='off';fig,ax=plt.subplots(1,2,figsize=(12,6));ax[0].imshow(original_image);ax[0].set_title('Original Image');ax[0].axis(A);ax[1].imshow(extracted_image);ax[1].set_title(f"Extracted Region ({round(accuracy_percentage,2)}%)");ax[1].axis(A);plt.show()
def export_predictions_to_csv(results,filename=_T,output_file=''):
	import csv
...
def preprocess_image(image_name,model,transform):
	if image_name.endswith('.jpg')or image_name.endswith('.png'):image_path=os.path.join(image_folder,image_name);original_image=Image.open(image_path).convert(_N);mask=segment_image(original_image,model,transform);extracted_image=extract_region(original_image,mask);return original_image,extracted_image
import pandas as pd
...
def evaluate_prediction(image_name,point_name,model,transform,image_folder,data_folder):
	image_path=os.path.join(image_folder,image_name);image=Image.open(image_path).convert(_N);mask=segment_image(image,model,transform);point_path=os.path.join(data_folder,point_name)
...
def evaluate_all_images(data_df,image_folder,data_folder,model,transform,export_output='',show_image=_C):
	results=[]
	for(idx,row)in data_df.iterrows():
		image_name=row[_O];shape_name=row['SHAPE'];point_name=row['POINT'];mean_distance,accuracy_percentage=evaluate_prediction(image_name,point_name,model,transform,image_folder,data_folder);results.append((image_name,accuracy_percentage,mean_distance))
	...
	if export_output!='':export_predictions_to_csv(results,filename=_T,output_file=export_output)
	return results
def evaluate_random_image(data_df,image_folder,data_folder,model,transform,num_samples=1,export_output='',show_image=_C):
...
def plot_accuracy_scatter(results,show=True):
	...
	if show:plt.show()
	return plt
if _U in locals():plot_accuracy_scatter(results)
def plot_accuracy_pie(results,show=True):
	results.sort(key=lambda x:x[0]);accuracies=[result[1]for result in results];good_average_accuracy=sum(accuracies)/len(accuracies);bad_average_accuracy=100-good_average_accuracy;labels=['Predicted  Accuracy ({}%)'.format(good_average_accuracy),'Predicted Error ({}%)'.format(bad_average_accuracy)];sizes=[good_average_accuracy,bad_average_accuracy];colors=[_V,'lightcoral'];plt.figure(figsize=(8,8));plt.pie(sizes,labels=labels,colors=colors,autopct='%1.1f%%',startangle=140);plt.axis('equal');plt.title('Overall Accuracy Distribution')
	if show:plt.show()
	return plt
if _U in locals():plot_accuracy_pie(results)
def get_model(model_name):
	model_name=model_name.lower().split('_weights')[0];model_mapping={_F:models.segmentation.deeplabv3_resnet50,_I:models.segmentation.deeplabv3_resnet50,_H:models.segmentation.deeplabv3_resnet101,_G:models.segmentation.deeplabv3_mobilenet_v3_large,_J:models.segmentation.fcn_resnet50,_L:models.segmentation.fcn_resnet50,_K:models.segmentation.fcn_resnet101,_M:models.segmentation.lraspp_mobilenet_v3_large,_E:models.segmentation.lraspp_mobilenet_v3_large};weights_mapping={_F:models.segmentation.DeepLabV3_ResNet50_Weights.DEFAULT,_I:models.segmentation.DeepLabV3_ResNet50_Weights.DEFAULT,_H:models.segmentation.DeepLabV3_ResNet101_Weights.DEFAULT,_G:models.segmentation.DeepLabV3_MobileNet_V3_Large_Weights.DEFAULT,_J:models.segmentation.FCN_ResNet50_Weights.DEFAULT,_L:models.segmentation.FCN_ResNet50_Weights.DEFAULT,_K:models.segmentation.FCN_ResNet101_Weights.DEFAULT,_M:models.segmentation.LRASPP_MobileNet_V3_Large_Weights.DEFAULT,_E:models.segmentation.LRASPP_MobileNet_V3_Large_Weights.DEFAULT}
...
def compare_models(available_models,data_df,csv_filename='compare_models'):
	E='execution_time';D='mean_distance';C='accuracy_percentage';B='image_name';A='model_name';import time,csv;csv_path=f"{output_folder}/{csv_filename}.csv";data_df=data_df.drop_duplicates()
	if os.path.exists(csv_path):existing_results=pd.read_csv(csv_path)
	else:
		with open(csv_path,mode='w',newline='')as file:writer=csv.writer(file);dfcolumns=[A,B,C,D,E];writer.writerow(dfcolumns)
	...
	for model_name in available_models:
		try:
			...
			if not existing_results.empty:evaluated_images=existing_results[existing_results[A]==model_name][B].tolist();data_df_filtered=data_df[~data_df[_O].isin(evaluated_images)];print(f"Skipping {len(data_df)-len(data_df_filtered)} images already evaluated.")
			else:data_df_filtered=data_df
			previous_results=existing_results[existing_results[A]==model_name]
			for(_,row)in previous_results.iterrows():
				image_name=row[B];accuracy_percentage=row[C];mean_distance=row[D];execution_time=row[E]
			...
			for(idx,row)in data_df_filtered.iterrows():
				image_name=row[_O];point_value=row['POINT'];i_counter+=1
				try:
			...
			if local_result:plt1=plot_accuracy_pie(local_result,show=_C);plt1.savefig(f"{os.path.join(output_folder,model_name)}_pie.png");plt2=plot_accuracy_scatter(local_result,show=_C);plt2.savefig(f"{os.path.join(output_folder,model_name)}_scatter.png")
...
def plot_model_comparison(model_results):
	import pandas as pd,matplotlib.pyplot as plt;from statistics import mean;df_results=pd.DataFrame({_A:[model for model in model_results],_D:[mean([result[1]for result in model_results[model]])for model in available_models],_B:[sum([result[3]for result in model_results[model]])for model in available_models]});plt.figure(figsize=(14,6));plt.subplot(1,2,1);bars=plt.bar(df_results[_A],df_results[_D],color='skyblue');plt.xlabel(_A);plt.ylabel(_D);plt.title('Model Accuracy Comparison');plt.xticks(rotation=45,ha=_P);plt.yticks(range(0,101,10))
	for bar in bars:yval=bar.get_height();plt.annotate(f"{yval}%",xy=(bar.get_x()+bar.get_width()/2,yval),xytext=(0,-125),textcoords=_Q,ha=_R,va=_S,rotation=90)
	plt.subplot(1,2,2);bars=plt.bar(df_results[_A],df_results[_B],color='salmon');plt.xlabel(_A);plt.ylabel(_B);plt.title('Model Execution Time Comparison');plt.xticks(rotation=45,ha=_P);min_time=min(df_results[_B]);max_time=max(df_results[_B]);ymin=min_time-8;ymax=max_time+1001;yminlim=min_time-100;ymaxlim=max_time+1000;ystep=450;plt.ylim(yminlim,max_time+500);plt.yticks(range(int(ymin),int(ymax),ystep))
	for bar in bars:yval=bar.get_height();plt.annotate(f"{yval:.2f}",xy=(bar.get_x()+bar.get_width()/2,yval),xytext=(0,5),textcoords=_Q,ha=_R,va=_S,rotation=0)
	plt.tight_layout();plt.show()
plot_model_comparison(model_results)
def plot_model_performance(model_results):
	A='Performance Index';import pandas as pd,matplotlib.pyplot as plt;from statistics import mean;df_results=pd.DataFrame({_A:[model for model in model_results],_D:[mean([result[1]for result in model_results[model]])for model in available_models],_B:[sum([result[3]for result in model_results[model]])for model in available_models]});df_results[A]=df_results[_D]/df_results[_B]*100000;df_results_sorted=df_results;plt.figure(figsize=(10,6));bars=plt.bar(df_results_sorted[_A],df_results_sorted[A],color=_V);plt.xlabel(_A);plt.ylabel(A);plt.title('Model Performance Comparison');plt.xticks(rotation=45,ha=_P);min_performance=df_results_sorted[A].min();max_performance=df_results_sorted[A].max();ymax=max_performance+6299+1;ymin=min_performance-780;yminlim=min_performance-100;ymaxlim=max_performance+1000;ystep=5000;plt.ylim(yminlim,ymaxlim);plt.yticks(range(int(ymin),int(ymax),ystep))
	for bar in bars:yval=bar.get_height();plt.annotate(f"{yval:.0f}",xy=(bar.get_x()+bar.get_width()/2,yval),xytext=(0,2),textcoords=_Q,ha=_R,va=_S,rotation=0)
	plt.tight_layout();plt.show()
